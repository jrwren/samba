/*
   smbput: a put utility with support for recursive uploading to
	smb:// urls
   Copyright (C) 2003-2004 Jelmer Vernooij <jelmer@samba.org>
   Copyright (C) 2017 Jay R. Wren <jrwren@xmtp.net>
...
static bool smb_upload_file(const char *name, const char *target);
...
static bool smb_upload_dir(const char *base, const char *name)
{
...
/*			return smb_download_file(base, name, true, resume,
	...
	while ((dirent = smbc_readdir(dirhandle))) {
		...
		case SMBC_DIR:
			ok = smb_upload_dir(base, newname);
			break;

		case SMBC_WORKGROUP:
			ok = smb_upload_dir("smb://", dirent->name);
			break;
		...
		case SMBC_FILE:
/*			ok = smb_upload_file(base, newname, true, resume,
						false, NULL);*/
			break;
...
static bool smb_upload_file(const char *name, const char *target)
{
	...
	snprintf(path, SMB_MAXPATHLEN-1, "%s%s%s", target,
		 (*target && *name && name[0] != '/' &&
		  target[strlen(target)-1] != '/') ? "/" : "",
		 basename);
    fprintf(stderr, "trying creat %s\n", path);


    //O_CREAT | O_NONBLOCK | O_RDWR | O_TRUNC
	//remotehandle = smbc_open(path, O_CREAT | O_WRONLY | O_TRUNC, 0755);
	remotehandle = smbc_creat(path, 0755);
	...
	if (remotehandle < 0) {
		...
		case ENOENT:
			fprintf(stderr,
				"%s or part of the path can't be found on the remote server\n",
				path);
			return false;
	...
	if (opt.update) {
		...
		localhandle = open(name, O_RDONLY, 0775);
		if (localhandle < 0) {
		...
		localhandle = open(name, O_RDONLY, 0755);
		if (localhandle < 0) {
			fprintf(stderr, "Can't open %s: %s\n", name,
				strerror(errno));
			smbc_close(remotehandle);
		...
		start_offset = remotestat.st_size;

		if (localstat.st_size &&
		    localstat.st_size == remotestat.st_size) {
			if (opt.verbose) {
				fprintf(stderr, "%s is already put "
					"completely.\n",
					path);
			} else if (!opt.quiet) {
				fprintf(stderr, "%s\n", path);
		...
		if (localstat.st_size > RESUME_CHECK_OFFSET &&
			...
			offset_download =
			    remotestat.st_size - RESUME_DOWNLOAD_OFFSET;
			offset_check = remotestat.st_size - RESUME_CHECK_OFFSET;
			if (opt.verbose) {
				printf("Trying to start resume of %s at %jd\n"
				       "At the moment %jd of %jd bytes have "
				       "been retrieved\n",
				       name, (intmax_t)offset_check,
	...
	/* Now, put all bytes from offset_download to the end */
	for (curpos = offset_download; curpos < localstat.st_size;
	     curpos += opt.blocksize) {
		ssize_t bytesread;
		ssize_t byteswritten;

		bytesread = read(localhandle, readbuf, opt.blocksize);
		if(bytesread < 0) {
			fprintf(stderr,
		...
		byteswritten = smbc_write(remotehandle, readbuf, bytesread);
		...
		if (opt.dots) {
			...
			print_progress(name, start_time, time_mono(NULL),
				       start_offset, curpos,
				       remotestat.st_size);
		}
	}
...
static void clean_exit(void)
{
	...
	if (!opt.quiet) {
		fprintf(stderr, "put %s in %lu seconds\n", bs,
...
//		{"recursive",  'R', POPT_ARG_NONE,   NULL,             'R',  "Recursively download files" },
		...
		{"target",     't', POPT_ARG_STRING, NULL,             't', "target SMB path"},
...
/*		case 'R':
	...
	while ((file = poptGetArg(pc))) {
		if (!recursive) {
			ret = smb_upload_file(file, target);
		} else {
			ret = smb_upload_dir(file, target);
		}
	}
